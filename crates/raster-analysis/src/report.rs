    ...
    pub fn to_text(&self) -> String {
        ...
        if self.metrics.total_tile_raster_overhead_ns > 0 {
            ...
            lines.push(format!(
                ...
                format_duration(self.metrics.total_tile_external_input_resolve_ns)
            ));
            lines.push(format!(
                "  internal input resolve: {}",
                format_duration(self.metrics.total_tile_internal_input_resolve_ns)
            ));
            lines.push(format!(
                "  output store: {}",
                format_duration(self.metrics.total_tile_output_store_ns)
            ));
            lines.push(format!(
                "  trace serialization: {}",
                format_duration(self.metrics.total_tile_trace_serialize_ns)
            ));
            lines.push(format!(
                "  draft capture: {}",
                format_duration(self.metrics.total_tile_draft_capture_ns)
            ));
            lines.push(format!(
                "  other wrapper: {}",
                format_duration(self.metrics.total_tile_other_wrapper_ns)
            ));
        }
