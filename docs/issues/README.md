...
Index of `docs/issues/`. Last reviewed 2026-09-16.
...
| [`fraud-window-trace-ends`](./fraud-window-trace-ends.md) | 2026-09-11 | The 128-bit guarantee is per *window*, and neither end of a trace has one. At the head, detection works but the window the host builds cannot be proven: `verify` slices the committed fingerprint with `index.saturating_sub(window_size) + 1` (`raster-prover/src/trace.rs:884`) while the rolling buffer holds `index + 1` items, and `Fingerprint::from` declares `len = window_size` regardless (`:890`), so every divergence below `index == window_size` trips the guest's `"Window fingerprint is shorter than its declared length"` (`fraud_proof.rs:341-355`); `terminal_window` (`:943`) clamps correctly, so the two windows its doc comment calls identically constructed are not. At the tail, detection has no forward runway and degrades from 128 bits to `bits_per_item` — 2⁻¹ at `window_size >= 128`, unmentioned in `--fraud-proof-window-size`'s help. Not a soundness hole: a degenerate window fails closed. Head reproduction is preempted by a separate defect — `witness_record_inputs` panics on any top-level `SequenceStart` (`trace.rs:620-630`). | [`chain-fraud-proof`](../proposals/chain-fraud-proof.md), [`chain-io-commitment`](../proposals/chain-io-commitment.md), [`window-seed-reconstruction`](../proposals/window-seed-reconstruction.md), [`program-chain`](../proposals/program-chain.md) |
