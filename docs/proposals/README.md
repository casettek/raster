...
| [`clone-free-bindings`](./clone-free-bindings.md) | proposed 2026-09-24 — **options only, design not chosen** | — | whole. `select!` moves its base (`SelectSource::select(self, …)`), so every base read twice needs a `.clone()` — 37 `select!` bases, 6 `call!`-family arguments, 6 `storage!` references across `crates/` and `examples/`. Cloning an `AuthRef` copies a coordinate and means nothing for lineage, and `expr_root_ident` strips `.clone()` anyway, so removal is provenance-neutral: `commit.bin` must stay byte-identical, which is the acceptance test. Four options costed, recommending borrow-the-base first; picks up the scope [`sequence-grammar-closure`](./sequence-grammar-closure.md) §What this does not fix set aside, and retires the `.clone()` arm its phase 2 would otherwise have to keep |
...
| [`storage-write-cost`](./storage-write-cost.md) | Term 1 **implemented** 2026-09-21; Term 2 proposed — **measured** | — | whole; a storage `append` cost 194–363 µs and the frontier append that records it is **0.3%** of that. **Term 1 done**: the root recompute was not the clone it was blamed on but an unmemoized `Hashable::empty_root` refold — 496 hashes per fold against ~32 of real spine — now memoized, 54.0 → 3.2 µs/call (16.8x), program 3.6 → 1.17 ms, `commit.bin` byte-identical. Term 2 (the 256-deep coordinate index) remains, now 93% of an append. [`storage-role-split`](./storage-role-split.md) **landed 2026-09-24** and took the child's half away, so Term 2 now applies only to the recorder's index — which is real work, but the recorder is 1.02 s of a 12.59 s `prompt-prepare` run and its appends are under 1% of that. Measure before spending: the same profiling found a **cargo no-op build check at 8.4 s (67%)** and one recur sequence's `input_trace_ns` at **2.40 s (19%)** |
| [`storage-role-split`](./storage-role-split.md) | **implemented** (2026-09-24) — **measured** | `StorageManager` split into `ObjectStore` (the running program's) and `AuthenticatedObjectStore` (the recorder's); the child's frontier, coordinate index and roots are gone, along with dead `StoredObject::log_position` and uncalled `global_storage_snapshot()`. `commit.bin` byte-identical after each of the three steps — the child's roots never reached a commitment, now shown rather than argued. On `prompt-prepare` (1275 tiles) `output_store_ns` **81.08 → 3.01 ms, −96.3%**, and tile time fell 73% because storing was 79% of it | **it is 0.6% of a run** — a wall-clock A/B measured −0.10%, inside noise. Read as a structural change, not a performance one. The budget it sits in: cargo no-op build check **8.4 s (67%)**, `input_trace_ns` in one recur sequence **2.40 s (19%)**, recorder **1.02 s (8%)** — which runs even with no `--commit`, its whole product consumed by a 3 ms print loop. All three are unowned and each dwarfs this proposal and `storage-write-cost` Term 2. Also: `--fraud-step 22` no longer fails fast, so this proposal's §Verification is stale |
| [`incremental-draft-materialization`](./incremental-draft-materialization.md) | proposed 2026-09-21 | — | whole; the *payload and index* half of what [`incremental-draft-witness`](./incremental-draft-witness.md) did for the digest. Verified: `AppendFrontier`'s root already equals the list root, and `finalize` then rebuilds it — **every element is hashed twice**. Two blockers found, both in layout: `RasterNode.offset` is absolute and struct fields are laid out sequentially, so appending to a non-last field shifts everything after it. Also carries the lifetime change — the wrapping sequence closes a draft, `finalize` leaves the grammar — which subsumes [`draft-provenance`](./draft-provenance.md), answers both of [`recur-deferred-finalize`](./recur-deferred-finalize.md)'s open questions, and gives [`authenticated-chain-draft-output`](../issues/authenticated-chain-draft-output.md) the trace step it is missing. **Revised 2026-09-24**: update-at-coordinate promoted from ruled-out cost note to the *residence model* — a draft occupies the coordinate of the recur unit that opens it, from the site's `Start` step to its seal, with the log appended per modification and the index advanced once. The "half-built object is observable" objection does not hold (no observer exists inside a sweep, and the index has not moved), and the site `Start` supplies the creation step the lifetime rule needs |
