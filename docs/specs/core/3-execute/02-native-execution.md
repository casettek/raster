...
- **Sequence discovery (native-only convenience)**
  - `crates/raster-macros/src/lib.rs` (`#[sequence]` macro wrapper generation)
  - `crates/raster-compiler/src/sequence.rs` (`SequenceDiscovery`)
...
- **Native tile invocation**
  - Confirm the backend/runtime actually calls the generated `__raster_tile_entry_*` wrapper.
...
- The host MUST invoke the tile-specific ABI wrapper generated by `#[tile]`.
- The wrapper has the byte-oriented signature:
  - `fn(&[u8]) -> Result<Vec<u8>>`
- A native runner MUST invoke a tile by:
  - Encoding the tile input with `postcard`.
  - Calling the generated wrapper symbol for that tile.

#### ABI serialization format (what the wrapper expects)

For host-side invocation through the ABI wrapper:
...
#### Example: sequence discovery vs execution
...
- The sequence MUST be discoverable by host-side AST/source discovery.
