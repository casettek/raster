...
- **Compiler/discovery failures (source scanning, caching, manifests)**:
  - `crates/raster-compiler/src/ast.rs` (AST parse + call extraction; some paths currently `unwrap`)
  - `crates/raster-compiler/src/tile.rs` and `crates/raster-compiler/src/sequence.rs` (tile/sequence discovery views)
...
- `#[tile]` **panics** (compile-time panic inside the proc macro) if:
  - the attribute includes `kind = ...` and the value is not one of `iter` or `recur`.
  - Note: an empty attribute like `#[tile]` is accepted and defaults `kind` to `iter`.
...
- `TileDiscovery::new(project)` and `SequenceDiscovery::new(project, ...)` run over an AST constructed by `ProjectAst::new(...)` (which parses `src/**/*.rs` with `syn`).
- If no tiles/sequences are found, tooling typically treats this as a non-error “empty discovery” case (e.g., the CLI prints “No tiles found”).
- Filesystem reads and parsing failures currently tend to surface as panics (`unwrap`) in the AST builder rather than structured `raster_core::Error` values. (This is a known robustness gap.)
