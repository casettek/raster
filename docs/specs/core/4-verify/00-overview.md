...
- **RISC0 receipt verification implementation**
  - Inspect `crates/raster-backend-risc0/src/risc0.rs`
    - Receipt encoding: `postcard::to_allocvec` / `postcard::from_bytes`
...
- **CLI “verify” entrypoint**
  - Inspect `crates/raster-cli/src/main.rs` and `crates/raster-cli/src/commands.rs`
    - `cargo raster run-tile --backend risc0 --tile <tile_id> --prove --verify` requests `ExecutionMode::Prove { verify: true }`.
...
- **Tile I/O trace items + commitment auditing (implemented; not a full verifier)**
  - Inspect `crates/raster-core/src/trace.rs` (`TraceItem`)
...
- The receipt encoding **MUST** be treated as `postcard` serialization of `risc0_zkvm::Receipt` (this is how Raster currently serializes/deserializes receipts).
...
- `guest_elf_bytes: [u8]`
- `receipt_bytes: [u8]` (`postcard`-serialized `risc0_zkvm::Receipt`)
...
- program-level trace commitments/fingerprints suitable for verifier protocols (window replay, fraud proofs),
- a divergence locator, window replay runner, or fraud proof object format.

Note: the workspace does provide a `TraceItem` commitment stream and a local `--audit` checker for native whole-program runs, but this is not yet integrated into a program verifier API.
...
```bash
cargo raster run-tile --backend risc0 --tile <tile_id> --prove --verify
```
