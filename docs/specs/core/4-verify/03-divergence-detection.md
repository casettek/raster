...
Raster’s current codebase provides only **partial** divergence-related functionality:

- A coarse, event-oriented `Trace` model exists (`Trace` / `TraceEvent`).
- A tile I/O transcript commitment stream exists for `TraceItem`s, and `--audit` can locate the **first mismatching item index** in a packed commitment stream.

However, Raster does **not** implement program-level divergence detection as described by the broader Verify specs (schema-driven step records, window replay selection/execution, fraud-proof object formats).
...
#### What exists today (trace data model + trace-item commitment audit)
...
- **Tile I/O trace items and commitment auditing (implemented)**
...
- **Hashing conventions**
  ...
  - Important: this spec documents that verifier-grade trace commitments/windowing are not implemented end-to-end today, even though a `TraceItem` commitment stream and `--audit` first-diff check exist.
...
- A `TraceItem` data model exists for tile I/O transcript items, plus a local “audit” mechanism that can locate the first divergence in a packed commitment stream:
  - `--audit` compares recomputed packed commitments against an expected file and identifies the first mismatching item index.
...
- program-level trace commitments that bind to schemas/artifact identities/entry inputs,
- window replay execution,
- verifier-consumable divergence proofs.
