...
- **How CFS is produced (binding resolution rules that determine what a verifier will see)**
  ...
  - `crates/raster-compiler/src/ast.rs`
    - `CallVisitor` extracts callsites as `CallInfo { callee, arguments, result_binding }` from parsed Rust syntax trees
...
**Current gap:** call-site `callee!(...)` intent is not captured by the current compiler/CFS generator (macro invocations are not extracted as calls), so verifiers cannot distinguish `callee(...)` vs `callee!(...)` from the CFS. Treating recursion as a tile-level property is the only mechanically available rule today, and even that is only metadata (`TileDef.type = "recur"`).
...
**Current gap:** the `#[tile(kind = recur)]` macro does not enforce either constraint at compile time.
