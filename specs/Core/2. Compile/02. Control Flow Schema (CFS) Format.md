...
- **Source discovery and call parsing (inputs, recursion marker)**
  - `crates/raster-compiler/src/discovery.rs`
    - `TileDiscovery` (scans `root/src/**.rs`, parses `#[tile(...)]` + fn signature)
    - `SequenceDiscovery` (scans `root/src/**.rs`, parses `#[sequence(...)]` + fn body)
    - `SequenceCall { arguments, result_binding, is_recursive }` (`callee!(...)` sets `is_recursive = true`)
...
- **Recursion authoring signal (macro emission)**
  - `crates/raster-macros/src/lib.rs`
    ...
    - This `!` call form is what source discovery detects as “recursive execution” intent
...
- `type` is usually `"iter"` or `"recur"` (derived from `#[tile(iter)]` / `#[tile(recur)]`).
...
Raster supports authoring a “recursive execution” intent by calling a recursive tile with `!` syntax (e.g., `count_to!(...)`).

Current behavior and representation:

- Source discovery records this intent per call as `SequenceCall.is_recursive = true` when the call expression uses `callee!(...)`.
- **The emitted CFS does not currently include any field to represent whether a `SequenceItem` is recursive.**

As a result:

- Consumers MUST NOT assume they can determine recursive execution intent from the CFS alone.
- Producers/consumers that require this information need an out-of-band channel or a schema extension (not implemented).
...
- **Recursive call intent is not encoded**
  - The compiler detects `callee!(...)`, but `SequenceItem` has no `is_recursive` field and the builder drops that signal.
...
- **Argument parsing is not a full Rust parser**
  - Discovery/resolution are based on textual heuristics (e.g., bare identifiers before `(`, comma splitting with limited nesting rules).
  - Complex expressions and literals often become `external` bindings.
- **Discovery scope**
...
- The project `src` directory does not exist under the chosen root (current implementation errors with “Source directory not found: ...”).
- Source files cannot be read due to I/O errors.
