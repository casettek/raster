...
- **Compilation orchestration**
  ...
  - `crates/raster-compiler/src/discovery.rs`: source scanning for `#[tile]` / `#[sequence]` and call extraction.
  - `crates/raster-compiler/src/cfs_builder.rs`: `CfsBuilder` and project name extraction.
...
- **Execution + backend surface**
  ...
  - `crates/raster-runtime/src/{lib.rs,executor.rs,tracer.rs}`: `Executor` + `Tracer` implementations (currently partial).
...
- **CFS**
  ...
  - `pub fn extract_project_name(...) -> Result<String>`
...
### `raster-runtime` (std-only runtime helpers)
...
- **Runtime execution and tracing**
  - `raster-runtime::{Executor, Tracer, FileTracer, NoOpTracer}` are public, but `Executor::execute` is currently a stub and `FileTracer` does not write to disk yet.
...
- `#[tile(<tile_type>, ...optional_kv_pairs...)]`
- `<tile_type>` MUST be either `iter` or `recur`.
...
- When a tile is declared as `#[tile(recur, ...)]`, the macro also exports a `macro_rules!` macro with the same name as the function.
- That macro supports `tile_name!(args...)` syntax and expands to `tile_name(args...)` for native compilation.
- Tooling MAY interpret the presence of the `!` call syntax as a marker for “recursive execution” during CFS generation.
...
#[tile(iter, description = "Double a number", estimated_cycles = 1000)]
fn double(x: u64) -> u64 {
    x * 2
}
```
...
- **`#[tile]` attribute syntax mismatch with discovery and CLI template**
  - The proc macro requires a first argument `iter` or `recur` and will panic otherwise.
  - The source scanner in `raster-compiler` accepts `#[tile]` (no args) and defaults to `iter`.
  - The CLI `init` template currently emits `#[tile(description = "...")]` without the required `iter|recur` first argument.
...
- **Execution stack is not wired end-to-end**
  - `raster-runtime::Executor::execute` is a stub.
  - `raster-backend::NativeBackend::execute_tile` is a stub and does not execute via the registry.
  - `raster-runtime::FileTracer` does not write to disk yet and uses a placeholder timestamp.
