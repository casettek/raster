...
- **Window replay execution is not implemented yet.**
  ...
  - There is no trace commitment / fingerprint implementation in `raster-core`.
...
- **(Missing today)** Search for a window replay runner:
  - `crates/raster-runtime/src/executor.rs` (`Executor::execute`) is currently a stub and does not implement sequence execution, replay, or windowing.
...
- **Tracer implementation**
  - `crates/raster-runtime/src/tracer.rs`: `Tracer` trait plus `FileTracer` and `NoOpTracer`.
    - `FileTracer::finalize` currently builds an in-memory `Trace` and does **not** write it to disk (TODO).
    - `Trace.timestamp` is currently hardcoded to `0` (TODO).
...
- `crates/raster-backend-risc0/src/risc0.rs` serializes receipts using `bincode`.
...
- **A real executor / runner capable of replay and windowing**
  - Likely in `crates/raster-runtime/src/executor.rs` (currently TODO).
- **Trace persistence**
  - `FileTracer` must write a trace to disk and define a stable file format.
