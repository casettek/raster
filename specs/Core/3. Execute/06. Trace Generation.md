...
### Runtime tracing hook points (current status: stubbed / TODO)

- `crates/raster-runtime/src/tracer.rs`
  - `trait Tracer::{record_event, finalize}`
  - `FileTracer` (currently buffers events; **does not write to disk yet**)
  - `NoOpTracer`
- `crates/raster-runtime/src/executor.rs`
  - `Executor::execute(...)` (**TODO: no execution, no event emission yet**)
...
- `Trace.timestamp` **SHOULD** be the trace creation time in nanoseconds since the Unix epoch.
  - **Current implementation gap**: `FileTracer::finalize()` sets `timestamp = 0`.
...
**Current implementation gap**: `FileTracer` does not currently write this (or any) encoding to disk; it only returns an in-memory `Trace`.
...
**Current implementation gap**: no failure variants exist in `TraceEvent`, and the executor does not emit events at all. Today, failures are surfaced only as `Result::Err(...)` returns from backend/runtime entry points.
...
- There are currently **no emitters**:
  - `Executor::execute` is unimplemented and does not call `Tracer::record_event`.
- There is currently **no persistence**:
  - `FileTracer::finalize()` does not write to a file and uses a placeholder timestamp.
