...
- There is **no dedicated “audit execution” runner** today.
  - `crates/raster-runtime/src/executor.rs` is a stub (`Executor::execute` is TODO).
  - The CLI provides tile-level execution for the RISC0 backend (`cargo raster run --backend risc0`) and a best-effort “sequence preview” loop (`cargo raster preview`), but neither enforces a CFS contract at runtime.
- The closest implemented mechanism to “audit” is **proof verification** on the RISC0 backend:
  - `ExecutionMode::Prove { verify: true }` causes the backend to attempt receipt verification against the ELF’s computed image ID.
  - **Important current behavior**: a verification failure is surfaced as `verified = false` in the result; it does **not** fail the run with an error (gap vs intended audit semantics).
...
- **CLI mode selection and what it actually runs**
  - `crates/raster-cli/src/commands.rs`
    - `run(...)`: maps `--prove`/`--verify` to `ExecutionMode` and calls `backend.execute_tile(...)` for RISC0.
...
- **Sequence call parsing and data-flow resolution**
  - `crates/raster-compiler/src/discovery.rs`
    - `SequenceDiscovery` and `SequenceCall { callee, result_binding, arguments, is_recursive }`
