...
- **RISC0 artifact identity (method ID / image ID)**
  - `crates/raster-backend-risc0/src/risc0.rs`
    ...
    - `verify_receipt`: `bincode` deserialization of `risc0_zkvm::Receipt` and `receipt.verify(image_id)`.
...
- `receipt_bytes = bincode::serialize(&risc0_zkvm::Receipt)`
...
**Important gap (portable encoding):** the receipt bytes are *not* a stable, versioned interchange format. They are Rust `bincode` for an upstream type (`risc0_zkvm::Receipt`) and are not expected to be directly consumable by on-chain verifiers.
...
interface IRasterTileVerifier {
    ...
    /// proof: chain-verifier-specific proof bytes (NOT Raster's bincode Receipt).
...
**Gap:** Raster does not emit `proof` in this form today; it emits `receipt_bytes` as `bincode(Receipt)`. A relayer would need to deserialize the receipt and translate it into the verifier’s calldata format.
...
- **Receipt encoding is not a contract ABI**
  - Raster serializes `risc0_zkvm::Receipt` with Rust `bincode`.
