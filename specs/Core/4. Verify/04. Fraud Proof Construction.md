...
- `crates/raster-backend-risc0/src/risc0.rs`
  ...
  - `Risc0Backend::execute_tile`
    ...
    - Receipt bytes encoding: `bincode::serialize(&risc0_zkvm::Receipt)`
  - `Risc0Backend::verify_receipt`
    - Receipt decoding: `bincode::deserialize::<risc0_zkvm::Receipt>(receipt_bytes)`
...
- **`receipt_bytes`**: a byte string equal to `bincode::serialize(&risc0_zkvm::Receipt)`.
...
Raster MUST encode `receipt_bytes` as `bincode::serialize(&risc0_zkvm::Receipt)`.

Raster MUST decode `receipt_bytes` as `bincode::deserialize::<risc0_zkvm::Receipt>(receipt_bytes)`.

**Gap (stability):**

`bincode` encoding is not specified as a stable, versioned wire format. Receipt bytes are currently an implementation detail and are not guaranteed to be portable across Raster/RISC0 versions.
...
- receipt bytes cannot be deserialized as `risc0_zkvm::Receipt` (`bincode` decode failure)
