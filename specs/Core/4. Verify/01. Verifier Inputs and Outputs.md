...
- **RISC0 receipt verification implementation**
  - Inspect `crates/raster-backend-risc0/src/risc0.rs`
    - Receipt serialization: `bincode::{serialize, deserialize}`
...
- **Tracing runtime**
  - Inspect `crates/raster-runtime/src/tracer.rs`
    - `FileTracer::finalize` currently does not write to disk (TODO).
...
Raster’s only implemented “verification” is **tile-level zkVM receipt verification** via the RISC0 backend. There is no program/sequence verifier, no “what-must-run-next” verifier, no divergence-point output, and no trace commitment verification in the current code.
...
- Receipt bytes are produced as `bincode::serialize(&risc0_zkvm::Receipt)`.
- Receipt bytes are consumed by verification as `bincode::deserialize::<risc0_zkvm::Receipt>(receipt_bytes)`.

Therefore:
- A verifier implementation that matches the current Raster code **MUST** treat the receipt encoding as **Rust `bincode` of the RISC0 `Receipt` type**.
...
- There is no on-disk trace format and no trace commitment/fingerprint implementation in current code.
...
- **`receipt_bytes: bytes`** (Rust `bincode` serialization of `risc0_zkvm::Receipt`)

### Verification algorithm (RISC0 backend)
An implementation matching the current code performs:
- Deserialize: `receipt = bincode::deserialize::<risc0_zkvm::Receipt>(receipt_bytes)`
...
- Because the receipt is `bincode`-encoded, this input bundle is not stable across languages and is not specified for long-term storage. Raster currently provides no canonical, versioned envelope for these bytes.
...
- There is **no trace persistence** and no trace commitment/fingerprint verification.
- Receipt encoding is **Rust `bincode`**, not a versioned, portable format.
