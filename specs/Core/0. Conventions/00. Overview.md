...
- **CFS construction from source scanning**
  ...
  - `crates/raster-compiler/src/discovery.rs` (source-based tile + sequence discovery)
...
### A.5 Traces (data model exists; runtime capture is incomplete)

- **Trace data model**
  - `crates/raster-core/src/trace.rs`
- **Runtime tracer plumbing**
  - `crates/raster-runtime/src/tracer.rs` (collects events; file output is TODO)
  - `crates/raster-runtime/src/executor.rs` (sequence execution is TODO)
...
- Tiles **MAY** return a plain value or a `Result<T, E>`. If the tile returns a `Result`, the wrapper forwards `Err` by converting it into `raster_core::Result` (exact behavior depends on the tile’s error type and current macro expansion).
...
- The RISC0 backend currently serializes receipts using **`bincode`**.
...
Source discovery can detect a `foo!(...)` call and records an `is_recursive` flag internally, and tiles may be declared with `#[tile(recur)]`. However:
